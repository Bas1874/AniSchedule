...
export async function fetchDubSchedule() {
    ...
    if (timetables) {
        ...
        for (const entry of currentSchedule) {
            ...
            if (((existingInAiring === -1) || (new Date(timetables[existingInAiring].delayedFrom) < new Date() && new Date(timetables[existingInAiring].delayedFrom).getUTCFullYear() > 1 && new Date(timetables[existingInAiring].delayedUntil).getUTCFullYear() <= 1)) && (existingInAiring !== -1 || entry.verified) && entry.episodeNumber < (entry.episodes || Infinity)) {
            ...
            } else if ((existingInAiring === -1) && !entry.verified && !(entry.episodeNumber < 2) && !(daysAgo(new Date(entry.episodeDate)) >= 1 && getCurrentDay() === 1)) {
            ...
            } else if ((existingInAiring !== -1) && (weeksDifference(timetables[existingInAiring].delayedFrom, past(new Date(), 0, true)) <= 4) && (new Date(timetables[existingInAiring].delayedFrom) > new Date(timetables[existingInAiring].delayedUntil))) {
        ...
        airingLists.value = timetables.filter(item => !item.airType || item.airType === 'dub').sort((a, b) => a.title.localeCompare(b.title));
    ...
    airing.forEach((entry) => {
        if (entry.route === 'kimetsu-no-yaiba-movie-mugen-jou-hen') {
    ...
    for (const parseObj of parseObjs) {
        ...
        const threshold = parseObj?.anime_title?.length > 15 ? 0.2 : parseObj?.anime_title?.length > 9 ? 0.15 : 0.1;
        ...
        if (!media || verification) {
            ...
            for (const parseObjAlt of fallbackTitles) {
                ...
                if (mediaAlt && !altVerification) {
                    ...
                    break;
                } else if (attempt === altTitles.length && (!mediaAlt || altVerification)) {
...
export async function updateDubFeed(optSchedule) {
    ...
    schedule.forEach(entry => {
        ...
        if (latestEpisodeInFeed && !dayTimeMatch(new Date(latestEpisodeInFeed.episode.airedAt), new Date(entry.episodeDate)) && (!entry.subtractedEpisodeNumber || (entry.subtractedEpisodeNumber > 1 && !((entry.episodeNumber - entry.subtractedEpisodeNumber) >= 6)))) {
            ...
            mediaEpisodes.sort((a, b) => b.episode.aired - a.episode.aired);
    ...
    const newEpisodes = (await Promise.all(schedule.map(async (entry) => {
        ...
        for (let episodeNum = lastFeedEpisode + 1; episodeNum < latestEpisode; episodeNum++) {
            ...
            const previousWeek = (await fetchPreviousWeek())?.find((airingItem) => airingItem.route === entry.route);
            const multiHeader = entry.subtractedEpisodeNumber || (previousWeek && previousWeek.subtractedEpisodeNumber);
            episodeType = multiHeader && baseEpisode && (lastFeedEpisode + 1 !== entry.subtractedEpisodeNumber) ? 2 : multiHeader ? 1 : 0;
            if (!baseEpisode && latestEpisode > episodeNum) {
                let weeksAgo = -1;
    ...
    if (JSON.stringify(newFeed) !== JSON.stringify(exactFeed || {})) {
        const lastUpdated = loadJSON(path.join('./raw/last-updated.json'));
        saveJSON(path.join('./raw/dub-episode-feed.json'), newFeed);
        saveJSON(path.join('./readable/dub-episode-feed-readable.json'), newFeed, true);
...
if (args[0] === 'update-dubs') {
    (async () => {
        try {
            ...
            // We need to pass the result of fetchDubSchedule to updateDubFeed
            const newSchedule = loadJSON(path.join('./raw/dub-schedule.json'));
            const feedChanges = await updateDubFeed(newSchedule);
            const allChanges = [...changes, ...feedChanges];
            
            if (allChanges.length > 0) {
                const { writeFile } = await import('node:fs/promises');
                await writeFile('./changes.txt', allChanges.join('\n'));
            }
        } catch (e) {
